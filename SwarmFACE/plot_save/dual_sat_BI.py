...
def save_dual_sat_BI(j_df, param):
    '''
    Save to ASCII file results from dual-satellite BI algorithm. Input from 
    j2satBI.py
    '''    
    ...
    fname_out = 'FAC_LR_sw'+''.join(sats)+'_BI_'+str_trange +'.dat'
    ...
    text_header = text_beg + 'angBN [deg]\n'
...
def plot_dual_sat_BI(j_df, dat_df, param):
    '''
    Plot results from dual-satellite BI algorithm. Input from 
    j2satBI.py
    '''
    ...
    fname_fig = 'FAC_LR_sw'+''.join(sats)+'_BI_'+str_trange +'.eps'    
    ...
    fig.suptitle('FAC density estimate with dual satellite BI method\n', size='xx-large',\
                 weight = 'bold', y=0.99)
    ...
    ratp = np.array([1, 1, 0.7, 1.5, 0.7 ]) #relative hight of each panel 
    ...
    ax[2].plot(j_df['angBN'])
    ax[2].set_ylabel('angBN\n[deg]', linespacing=1.7)
    
    if use_filter:
        ax[3].plot(j_df['FAC_flt'], label='$\mathrm{J_{BI \_ '+''.join(sats)+' \_ flt}}$')
    else:
        ax[3].plot(j_df['FAC'], label='$\mathrm{J_{BI \_ '+''.join(sats)+'}}$')
    ax[3].plot(FAC_L2['FAC'], label='$\mathrm{J_{Level2}}$')
    ax[3].axhline(y=0, linestyle='--', color='k', linewidth=0.7)
    ax[3].set_ylabel('$J_{FAC}$\n[$\mu A/m^2$]', linespacing=1.7)
    ax[3].legend(loc = (0.95, 0.1), handlelength=1)

    if use_filter:
        ax[4].plot(j_df['FAC_flt_er'])
    else:
        ax[4].plot(j_df['FAC_er'])
