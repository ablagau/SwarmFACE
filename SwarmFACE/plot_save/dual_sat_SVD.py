...
def save_dual_sat_SVD(j_df, param):
    ...
    fname_out = 'FAC_LR_sw'+''.join(sats)+'_SVD_'+str_trange +'.dat'
    ...
    text_header = text_beg + 'angBN [deg],\t  AD,\t  tau \n'
    ...
    with open(fname_out, 'w') as file:
        ...
        file.write('# Degeneracy start threshold: ' + str(tauast) + ' \n')   
        file.write('# Degeneracy end threshold: ' + str(taunul) + ' \n')      
        file.write('# Degeneracy interpolation: ' + str(intpol) + ' \n')                   
...
def plot_dual_sat_SVD(j_df, dat_df, param):
    ...
    fname_fig = 'FAC_LR_sw'+''.join(sats)+'_SVD_'+str_trange +'.eps'    
    ...
    fig.suptitle('FAC density estimate with dual satellite SVD method\n', size='xx-large',\
                 weight = 'bold', y=0.99)
    ...
    ratp = np.array([1, 1, 0.5, 0.5, 0.7, 1.5, 0.7 ]) #relative hight of each panel 
    ...
    ax[4].plot(j_df['AD'], label='$AD$')
    ax[4].plot(j_df['tau'], label='$tau$')
    ax[4].set_ylabel('AD/ tau\n', linespacing=1.7)
    ax[4].axhline(y=1, linestyle='--', color='k', linewidth=0.7)
    ax[4].set_ylim(bottom = -0.1, top = 2.1)
    ax[4].locator_params(axis='y', nbins=3)
    ax[4].legend(loc = (0.95, 0.1), handlelength=1)
    
    if use_filter:
        ax[5].plot(j_df['FAC_flt'], label='$\mathrm{J_{SVD \_ '+''.join(sats)+' \_ flt}}$')
    else:
        ax[5].plot(j_df['FAC'], label='$\mathrm{J_{SVD \_ '+''.join(sats)+'}}$')
